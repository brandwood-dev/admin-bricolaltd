...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const dateRangeParams = dateRange
          ...
          : undefined
        const response =
          await dashboardService.getDashboardData(dateRangeParams)
        if (response.success) {
    ...
    const fetchUnifiedData = async () => {
      try {
        ...
        const dateRangeParams = dateRange
          ...
          : {}
        const analyticsResp = await analyticsService.getAnalyticsData({
          ...dateRangeParams,
          period: selectedPeriod,
        })
        if (analyticsResp.success) {
          setAnalyticsData(analyticsResp.data)
          console.log('🔍 Analytics Data', analyticsResp.data)
        }
        const toolsResp = await toolsService.getToolStats()
        setToolStats(toolsResp.data)
        try {
          const usersCountryResp = await analyticsService.getUsersByCountry()
          if (usersCountryResp.success) {
            setUsersByCountry(usersCountryResp.data)
          } else {
            setUsersByCountry([])
          }
        } catch (e) {
          console.warn('Users by country endpoint unavailable, skipping.')
          setUsersByCountry([])
        }
        const disputesResp = await disputesService.getDisputeStats(
          dateRange
            ? {
        ...
        const newsResp = await newsService.getNews({
          isPublic: true,
          page: 1,
          limit: 1,
        })
        ...
        toast({
          title: 'Erreur',
          description: 'Impossible de charger les analytiques',
          variant: 'destructive',
        })
      } finally {
    ...
    ? [
        {
          ...
          changeType:
            analyticsData.kpis.revenue_growth >= 0
              ? ('positive' as const)
              : ('negative' as const),
          icon: DollarSign,
          color: 'bg-green-500',
        },
        {
          ...
          changeType:
            analyticsData.kpis.user_growth >= 0
              ? ('positive' as const)
              : ('negative' as const),
          icon: Users,
          color: 'bg-blue-500',
        },
        {
          ...
          value: (
            toolStats?.total ||
            analyticsData.kpis.active_tools ||
            0
          ).toLocaleString(),
          change: `${analyticsData.kpis.tool_growth >= 0 ? '+' : ''}${
            analyticsData.kpis.tool_growth
          }%`,
          changeType:
            analyticsData.kpis.tool_growth >= 0
              ? ('positive' as const)
              : ('negative' as const),
          icon: Package,
          color: 'bg-orange-500',
        },
        {
          ...
          changeType:
            analyticsData.kpis.booking_growth >= 0
              ? ('positive' as const)
              : ('negative' as const),
          icon: Calendar,
          color: 'bg-purple-500',
  ...
  const topToolsLimited = (topToolsData || []).slice(0, 6)
        ...
        <TabsContent value='revenue' className='space-y-6'>
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
            ...
            <Card>
              ...
              <CardContent>
                <ChartContainer
                  config={{ category: { label: 'Catégorie' } }}
                  className='h-80'
                >
                  <ResponsiveContainer width='100%' height='100%'>
                    <RechartsPieChart>
                        ...
                        content={
                          <ChartTooltipContent
                            formatter={(value, name, item, index, payload) => {
                              ...
                              const cat = payload?.category || item?.name
                              const totalRev =
                                analyticsData?.kpis.total_revenue || 0
                              const revenue =
                                payload?.revenue !== undefined
                                  ? payload.revenue
                                  : Math.round((pct / 100) * totalRev)
                              return (
                ...
                <div className='space-y-6'>
                  <div className='grid grid-cols-2 gap-4'>
                    <div className='text-center p-4 bg-blue-50 rounded-lg'>
                      <p className='text-2xl font-bold text-blue-600'>
                        {(
                          analyticsData?.kpis.active_users || 0
                        ).toLocaleString()}
                      </p>
                  ...
                  <div className='pt-2'>
                    <Card>
                      ...
                      <CardContent>
                        <ChartContainer
                          config={{ users: { label: 'Utilisateurs' } }}
                          className='h-80'
                        >
                          <ResponsiveContainer width='100%' height='100%'>
                            <RechartsPieChart>
            ...
            <Card>
              ...
              <CardContent>
                <div className='space-y-4'>
                  {topToolsLimited.map((tool, index) => (
                    <div
                      key={index}
