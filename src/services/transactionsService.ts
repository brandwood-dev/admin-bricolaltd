...
class TransactionsService {
  ...
  async getTransactionStats(dateRange?: {
    ...
    return await apiClient.get<TransactionStats>('/admin/transactions/stats', {
      params,
    })
  }
  ...
  async getTransactionsByPeriod(
    ...
    ApiResponse<{ date: string; transactions: number; amount: number }[]>
  > {
    return await apiClient.get<
      { date: string; transactions: number; amount: number }[]
    >('/admin/transactions/by-period', {
      params: { period },
    })
  }

  async getTransactionsByType(): Promise<
    ApiResponse<{ type: TransactionType; count: number; amount: number }[]>
  > {
    return await apiClient.get<
      { type: TransactionType; count: number; amount: number }[]
    >('/admin/transactions/by-type')
  }

  async getTransactionsByStatus(): Promise<
    ApiResponse<{ status: TransactionStatus; count: number; amount: number }[]>
  > {
    return await apiClient.get<
      { status: TransactionStatus; count: number; amount: number }[]
    >('/admin/transactions/by-status')
  }
  ...
  async bulkUpdateTransactions(
    data: BulkTransactionAction
  ): Promise<ApiResponse<{ updated: number; failed: string[] }>> {
    return await apiClient.post<{ updated: number; failed: string[] }>(
      '/admin/transactions/bulk-update',
      data
    )
  }
  ...
  async getTransactionsByBooking(
    bookingId: string
  ): Promise<ApiResponse<Transaction[]>> {
    return await apiClient.get<Transaction[]>(
      `/admin/transactions/booking/${bookingId}`
    )
  }

  // Export functionality
  async exportTransactions(
    params?: TransactionFilterParams & { format: 'csv' | 'excel' }
  ...
  async getPaymentMethods(): Promise<
    ApiResponse<{ method: PaymentMethod; count: number; amount: number }[]>
  > {
    return await apiClient.get<
      { method: PaymentMethod; count: number; amount: number }[]
    >('/admin/transactions/payment-methods')
  }
  ...
  async getRevenueByPeriod(
    ...
    ApiResponse<{ date: string; revenue: number; fees: number; net: number }[]>
  > {
    const params = {
      period,
      ...(dateRange && {
    ...
    return await apiClient.get<
      { date: string; revenue: number; fees: number; net: number }[]
    >('/admin/transactions/revenue-by-period', { params })
  }
  async getByBookingId(bookingId: string): Promise<ApiResponse<Transaction[]>> {
    return await apiClient.get<Transaction[]>(
